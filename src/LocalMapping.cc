    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames() && !mbBadImu)
        {
            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
...
#endif
                // Turn off VI bundle adjustment AND scale refinement (mono)
                ...
                //         // scale refinement (if the number of keyframes is small and if monocular)
...
{
    std::cout << "INITIALIZING THE IMU." << std::endl; 

    if (mbResetRequested){
        std::cout << "Reset has been requested. Cancel IMU init." << std::endl;
        return;
    }
    ...
    if(mpAtlas->KeyFramesInMap()<nMinKF){
        std::cout << "Not enough keyframes in map. Cancel IMU init." << std::endl;
        return;
    }

    // Retrieve all keyframe in temporal order
    std::cout << "Retrieving keyframes in temporal order." << std::endl;
    list<KeyFrame*> lpKF;
    KeyFrame* pKF = mpCurrentKeyFrame;
    while(pKF->mPrevKF)
    {
    ...
    if(mpCurrentKeyFrame->mTimeStamp-mFirstTs<minTime){
        std::cout << "Not enough time elapsed to init IMU." << std::endl;
        return;
    }

    bInitializing = true; // At this point, all the conditions have been met to start the initialization.

    std::cout << "ORBSLAM now initializing the IMU !!" << std::endl;
    ...
    if (!mpCurrentKeyFrame->GetMap()->isImuInitialized())
    {
        ...
        for(vector<KeyFrame*>::iterator itKF = vpKF.begin(); itKF!=vpKF.end(); itKF++)
        {
            ...
            // std::cout << "IMU rotation: " << (*itKF)->mPrevKF->GetImuRotation() << std::endl; // IMU to camera ?
            // std::cout << "Delta vel: " << (*itKF)->mpImuPreintegrated->GetUpdatedDeltaVelocity() << std::endl; 
            (*itKF)->SetVelocity(_vel);
            (*itKF)->mPrevKF->SetVelocity(_vel);
        }

        dirG = dirG/cv::norm(dirG); // direction of gravity in the camera frame
        // std::cout << "Gravity direction in cam frame: " << dirG << std::endl;
        ...
        std::cout << "Transform: " << cvRwg << std::endl;
