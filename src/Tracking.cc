...
void Tracking::StereoInitialization()
{
    if(mCurrentFrame.N>500)
    {
        if (mSensor == System::IMU_STEREO)
        {
            ...
            if (cv::norm(mCurrentFrame.mpImuPreintegratedFrame->avgA-mLastFrame.mpImuPreintegratedFrame->avgA)<0.005)
            {
                cout << "not enough (change in) acceleration" << endl;
                return;
            }
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    if(((c1a||c1b||c1c))||c3 ||c4) //if(((c1a||c1b||c1c) && c2)||c3 ||c4) // eliminated c2
    {
